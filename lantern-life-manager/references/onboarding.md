...
Use a **framework-first** onboarding sequence. Ask exactly one personal question at a time, keep it short, and accept “skip for now,” “I don't know,” or a partial answer. Do not apply that conversational rule to mechanical setup. After the user answers a blocking question, continue through every safe, private, reversible setup action. Do not wait for the user to say continue.
...
3. whether this account is using the installed Skill or Project Kit path.
...
Always create a Life Manager Project. Create a separate domain Project only for areas the interview establishes as active or useful. A person can add another domain later without reorganizing existing ones.
